...
import { BookOpen, FileText, SunMoon, Compass, Home, Edit, ArrowLeft, ArrowRight, Laptop } from "lucide-react";
import { useTheme } from "next-themes";
import { cn } from "@/lib/utils";

// ── macOS Cursor SVGs ───────────────────────────────────────────────────────

// macOS Arrow Cursor: Black arrow with a crisp white border and drop shadow.
// Hotspot: tip of the arrow (offset by x: -10px, y: -7px)
const MacOSArrow = () => (
  <svg
    height="32"
    viewBox="0 0 32 32"
...
// macOS Pointing Hand Cursor: White glove pointing up.
// Hotspot: tip of the index finger (offset by x: -9px, y: -8px)
const MacOSHand = () => (
  <svg
    height="32"
    viewBox="0 0 32 32"
...
const CURSOR_CONFIG: Record<string, CursorSetting> = {
  ...
  theme: {
    icon: SunMoon,
    label: ({ resolvedTheme }) => (resolvedTheme === "dark" ? "Light Theme" : "Dark Theme"),
    color: (isDark) => (isDark ? "#facc15" : "#d97706"),
  },
  blog: {
    icon: BookOpen,
    label: "Read",
    color: (isDark) => (isDark ? "#c084fc" : "#7c3aed"),
  },
  resume: { icon: Icons.Resume, label: "Resume" },
  nav: {
    icon: Compass,
    label: ({ navLabel }) => navLabel,
    color: (isDark) => (isDark ? "#2dd4bf" : "#0d9488"),
  },
  home: {
    icon: Home,
    label: "Home",
  ...
  text: {
    icon: Edit,
    label: "Type",
    color: (isDark) => (isDark ? "#fb7185" : "#e11d48"),
  },
  "pagination-prev": {
    icon: ArrowLeft,
    label: "Previous",
...
export function CustomCursor() {
  ...
  const [isOnTopSide, setIsOnTopSide] = useState(false);
  ...
  const isOnRightSideRef = useRef(false);
  const isOnTopSideRef = useRef(false);
  ...
  useEffect(() => {
    ...
    window.addEventListener('emailCopied', handleEmailCopied);
    return () => window.removeEventListener('emailCopied', handleEmailCopied);
  }, []);
  ...
  useEffect(() => {
    const moveCursor = (e: MouseEvent) => {
      ...
      const isRight = e.clientX > window.innerWidth - 180;
      if (isRight !== isOnRightSideRef.current) {
        isOnRightSideRef.current = isRight;
        setIsOnRightSide(isRight);
      }

      const isTop = e.clientY < 40;
      if (isTop !== isOnTopSideRef.current) {
        isOnTopSideRef.current = isTop;
        setIsOnTopSide(isTop);
      }
    };
    ...
    const updateHoverState = (e: MouseEvent) => {
      ...
      const isCursorPointerClass = target.closest(".cursor-pointer");

      const anchor = target.closest("a");
      const button = target.closest("button") || target.closest('[role="button"]');
      const isClickableInput = target.closest('input[type="submit"]') || target.closest('input[type="button"]');
      const textInput = target.closest('input[type="text"], input[type="email"], input[type="search"], textarea, [contenteditable="true"]');
      const customCursorElement = target.closest('[data-cursor-type]');

      let newHoverType = "";

      if (customCursorElement) {
        newHoverType = customCursorElement.getAttribute("data-cursor-type") || "";
        if (newHoverType === "skill") {
          const sName = customCursorElement.getAttribute("data-skill-name") || "";
          setSkillName(sName);
        } else if (newHoverType === "nav") {
          const nLabel = customCursorElement.getAttribute("data-nav-label") || "";
          setNavLabel(nLabel);
        }
      } else if (textInput) {
        newHoverType = "text";
      } else if (isCursorPointerClass || anchor || button || isClickableInput) {
        const nearestInteractive = target.closest("a, button, [role='button'], input[type='submit'], input[type='button']");
        if (nearestInteractive) {
          const tagName = nearestInteractive.tagName.toLowerCase();
          if (tagName === "a") {
            const href = nearestInteractive.getAttribute("href") || "";
            if (href.startsWith("mailto:")) {
              newHoverType = "mail";
            } else if (href.includes("github.com")) {
              newHoverType = "github";
            } else if (href.includes("linkedin.com")) {
              newHoverType = "linkedin";
            } else if (href.includes("x.com") || href.includes("twitter.com")) {
              newHoverType = "x";
            } else if (href.includes("/blog/")) {
              newHoverType = "blog";
            } else if (href.endsWith(".docx") || href.includes("resume") || href.includes("Karan%20Wakhare")) {
              newHoverType = "resume";
          ...
          } else if (tagName === "button" || nearestInteractive.getAttribute("role") === "button") {
            const ariaLabel = nearestInteractive.getAttribute("aria-label") || "";
            const textContent = (nearestInteractive.textContent || "").trim().toLowerCase();
            if (ariaLabel.toLowerCase().includes("theme") || nearestInteractive.querySelector("svg") || nearestInteractive.closest(".ModeToggle")) {
              const svg = nearestInteractive.querySelector("svg");
              const hasSunOrMoon = svg && (ariaLabel.toLowerCase().includes("theme") || nearestInteractive.querySelector("[key='sun']") || nearestInteractive.querySelector("[key='moon']"));
              if (hasSunOrMoon || ariaLabel.toLowerCase().includes("theme")) {
                newHoverType = "theme";
              } else {
                newHoverType = "click";
              }
            } else if (textContent === "previous") {
              newHoverType = "pagination-prev";
            } else if (textContent === "next") {
              newHoverType = "pagination-next";
            } else if (nearestInteractive.closest("#main-nav")) {
              newHoverType = "nav";
              setNavLabel((nearestInteractive.textContent || "").trim());
            } else {
              newHoverType = "click";
            }
          } else {
      ...
      const isNowHovering = newHoverType !== "";
      
      setHoverType((prev) => (prev !== newHoverType ? newHoverType : prev));
      setIsHovering((prev) => (prev !== isNowHovering ? isNowHovering : prev));
    };
  ...
  useEffect(() => {
    const checkTouch = () => {
      // Only disable if the primary input is coarse (like a phone/tablet without a mouse)
      setIsTouchDevice(
  ...
  const config = CURSOR_CONFIG[hoverType];
  const showLabel = isHovering && hoverType && hoverType !== "pointer" && config;
  const labelText = showLabel
    ? typeof config.label === "function"
      ? config.label({ skillName, navLabel, isEmailCopied, resolvedTheme: resolvedTheme || "" })
      : config.label
    : "";
  const iconColor = showLabel && config.color
  ...
  return (
    <>
      ...
      {/* macOS Custom Cursor Container */}
      <motion.div
        className="fixed top-0 left-0 pointer-events-none z-[9999] hidden md:block"
        style={{
          x: cursorX,
          y: cursorY,
          translateX: isHovering ? "-9px" : "-10px",
          translateY: isHovering ? "-8px" : "-7px",
          scale: isMouseDown ? 0.88 : 1.0,
          willChange: "transform",
        }}
        transition={{
          translateX: { duration: 0.08, ease: "easeOut" },
          translateY: { duration: 0.08, ease: "easeOut" },
          scale: { type: "spring", stiffness: 500, damping: 20 },
        }}
      >
        {isHovering ? <MacOSHand /> : <MacOSArrow />}
      </motion.div>

      {/* Interactive text label next to cursor */}
      <AnimatePresence>
        {showLabel && (
          <motion.span
            className="fixed top-0 left-0 text-[10px] font-sans font-semibold tracking-wider uppercase text-popover-foreground select-none pointer-events-none z-[9999] bg-popover/90 backdrop-blur-xs border border-border px-2.5 py-1.5 rounded-md shadow-md hidden md:flex items-center gap-1.5 whitespace-nowrap"
            style={{
              x: cursorX,
              y: cursorY,
              translateX: isOnRightSide ? "-115%" : "24px",
              translateY: isOnTopSide ? "20px" : "-10px",
            }}
          ...
          >
            {config.icon && (
              <config.icon
                className={cn("size-3", config.iconClassName)}
